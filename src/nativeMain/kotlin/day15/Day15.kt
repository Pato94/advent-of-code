...
class Day15 {
    ...
    private inline fun h(grid: Array<IntArray>, x: Int, y: Int): Int {
        return (grid.size - 1 - y) + (grid[0].size - 1 - x)
    }

    private inline fun toKey(x: Int, y: Int): Int {
        return x * 1000 + y
    }

    private inline fun toX(key: Int): Int {
        return key / 1000
    }

    private inline fun toY(key: Int): Int {
        return key % 1000
    }
    ...
    private fun aStar(grid: Array<IntArray>): Int {
        ...
        while (openSet.isNotEmpty()) {
            val currentKey = openSet.minByOrNull { fScore[it] ?: Int.MAX_VALUE }!!
    ...
    private fun part1(): Int {
        val string = readAllText("./src/nativeMain/kotlin/day15/input.txt")
